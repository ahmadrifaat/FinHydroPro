...
float Voltage = 0.0;
const float V4 = 3.05;  // Tegangan untuk buffer pH 4
const float V7 = 2.45;  // Tegangan untuk buffer pH 7
...
void setup() {
  ...
  // Kalibrasi EC
  int adcStandard = 1450;  // Nilai ADC dari larutan standar
  float ecStandard = 790.0;  // Nilai EC larutan standar dalam μS/cm
...
void sendDataToFirebase(float suhu, float pH, float ec) {
  if (Firebase.ready() && signupOK && (millis() - sendDataPrevMillis > 5000)) {
    ...
    if (Firebase.RTDB.setJSON(&fbdo, "Monitoring/Room5", &json)) {
